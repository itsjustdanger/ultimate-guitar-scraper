...
function searchExtractTab ($, tr) {
  ...
  // Type.
  let $td4 = $(tds[3])
  let type = removeShit($td4.text().trim())
  if (type.length > 0) {
    tab['type'] = type
  }
  return tab
...
function parseSingleTAB (html) {
  ...
  if ($infoKeysElement && $infoValuesElement) {
    let infoKeys = $infoKeysElement.html().split(/<br\/?>/).map(function (text) {
      return text.trim()
    })
...
* @return {Object} formatted query params
*/
function formatAutocompleteQuery (query) {
  let params = {}
  let acceptedParams = ['query', 'type', 'artist']
  ...
  // conditions
  let autocompleteTypes = 'tab artist'.split(' ')
...
function encodeParams (params) {
  ...
  let encodedParams = Object.keys(params).map((key) => {
    ...
    if (Array.isArray(params[key])) {
      encodedParam = params[key].map((value) => {
